...
将大型项目拆为 8 个业务批次，再在每个批次内按行数（≤1400 行/批）进一步拆分 commit。
...
1. **打包上限 1400 行**（为 1500 硬限制留缓冲）
2. **单文件超大** → 独占一批；若 > 1500 行 → 标题类型 `skip`
3. **Test 文件** → 必须独立批次，不得与业务代码混合
4. **多语言 README** → 每种语言单独 commit（通常各 ~900 行）
...
| 超大单文件源码（如 TUI > 1500 行） | 单独 `skip` commit |
| 图片/二进制（PNG/JPG/SVG） | 按文本统计可能虚高；大图单独 `skip` commit |
| `yarn.lock` / `package-lock.json` | 通常 > 1500 行 → `skip` commit |
